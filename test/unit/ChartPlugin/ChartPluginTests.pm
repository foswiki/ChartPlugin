...
sub test_getTableRanges {
    ...
    $this->assert_deep_equals([[{ left => 0, right => 3,
				  top => 0, bottom => 2}]], \@r);
    @r = $t->getTableRanges("one", "R3:C4..R1:C1");
    $this->assert_deep_equals([[{ left => 3, right => 0,
				  top => 2, bottom => 0}]], \@r);
    @r = $t->getTableRanges("one", "R1:C1..R3:C2+R3:C4..R1:C3");
    $this->assert_deep_equals([[{ left => 0, right => 1,
				  top => 0, bottom => 2},
			       { left => 3, right => 2,
			       top => 2, bottom => 0 }]], \@r);
}
...
sub test_getData {
    ...
    $this->assert_deep_equals([[111,112,113,114],
			       [121,122,123,124],
			       [131,132,133,134]], \@r);
...
sub test_transpose {
	...
	$tx, [ Foswiki::Plugins::ChartPlugin::Table::transpose(
	    @$untx) ]);
}
