...
sub initPlugin {
    ...
    if ( $Foswiki::Plugins::VERSION < 1 ) {
        &Foswiki::Func::writeWarning(
            "Version mismatch between ChartPlugin and Plugins.pm");
        return 0;
    }
    ...
    &Foswiki::Func::writeDebug(
        "- Foswiki::Plugins::ChartPlugin::initPlugin($web.$topic) is OK")
      if $debug;
...
sub _init_defaults {
    ...
    $defaultType = Foswiki::Func::getPreferencesValue("CHARTPLUGIN_TYPE")
      || 'line';

    # Get default chart values
    $defaultWidth = &Foswiki::Func::getPreferencesValue("CHARTPLUGIN_WIDTH")
      || 60;
    $defaultHeight = &Foswiki::Func::getPreferencesValue("CHARTPLUGIN_HEIGHT")
      || 16;
    my $defaultAreaColors =
      &Foswiki::Func::getPreferencesValue("CHARTPLUGIN_AREA_COLORS")
      || "#FF0000 #FFFF00 #00FF00";
    @defaultAreaColors = split( /[\s,]+/, $defaultAreaColors );
    my $defaultLineColors =
      &Foswiki::Func::getPreferencesValue("CHARTPLUGIN_LINE_COLORS")
      || "#FFFF00 #FF00FF #00FFFF";
    @defaultLineColors = split( /[\s,]+/, $defaultLineColors );

    # Get default chart bgcolor
    $defaultBGcolor = &Foswiki::Func::getPreferencesValue("CHARTPLUGIN_BGCOLOR")
      || '#FFFFFF #FFFFFF';

    # Get default number of Y axis grids
    $defaultNumYGrids =
      &Foswiki::Func::getPreferencesValue("CHARTPLUGIN_NUMYGRIDS") || 10;

    # Get default value to use if there is no data seen in the table
    $defaultDataValue =
      &Foswiki::Func::getPreferencesValue("CHARTPLUGIN_DEFAULTDATA");
    ...
    $defaultGridColor =
      &Foswiki::Func::getPreferencesValue("CHARTPLUGIN_GRIDCOLOR") || '#000000';

    # Get default value for the size, in pixels, of drawn data points
    $defaultPointSize =
      &Foswiki::Func::getPreferencesValue("CHARTPLUGIN_POINTSIZE") || 2;

    # Get default value for the width, in pixels, of drawn lines
    $defaultLineWidth =
      &Foswiki::Func::getPreferencesValue("CHARTPLUGIN_LINEWIDTH") || 3;

    # Get default value for the leading space before the first bar.
    $defaultBarLeadingSpace =
      &Foswiki::Func::getPreferencesValue("CHARTPLUGIN_BARLEADINGSPACE") || 0;

    # Get default value for the trailing space after the last bar.
    $defaultBarTrailingSpace =
      &Foswiki::Func::getPreferencesValue("CHARTPLUGIN_BARTRAILINGSPACE") || 0;

    # Get default value for the space between bars.
    $defaultBarSpace =
      &Foswiki::Func::getPreferencesValue("CHARTPLUGIN_BARSPACE") || 0;
}
...
sub _setTopicContents {
    ...
    # contents.
    if (   ( $inWeb eq $this->{CURRENT_WEB} )
        && ( $inTopic eq $this->{CURRENT_TOPIC} ) )
    {
        ...
        ( my $meta, $topicContents ) =
          Foswiki::Func::readTopic( $inWeb, $inTopic );
        ...
        $topicContents =
          Foswiki::Func::expandCommonVariables( $topicContents, $inTopic,
            $inWeb );
    }

    # Lets parse the specified topic contents looking for tables.
    $this->_setTables(
        Foswiki::Plugins::ChartPlugin::Table->new($topicContents) );
    return 1;
}
...
sub _makeChart {
    ...
    return _make_error("parameter *type* must be specified")
      if ( !defined $type );
    ...
    return _make_error(
        "paramters *datatype* and *subtype* can't both be specified")
      if ( defined $dataType && defined $subType );
    ...
    if ( defined $subType ) {
        ...
        my @unknownSubTypes =
          grep( !/area|line|point|pline|scatter|bar/, @subTypes );
        return _make_error(
            "unknown subtypes: " . join( ", ", @unknownSubTypes ) )
          if (@unknownSubTypes);
        ...
        if ( $type eq "line" ) {
            ...
            return _make_error( "unsupported subtypes: "
                  . join( ", ", @unknownSubTypes )
                  . " for type line" )
              if (@unknownSubTypes);
        }
    ...
    if ( !$this->_setTopicContents( $inWeb, $inTopic ) ) {
        return _make_error(
            "Error retrieving Foswiki topic $inWeb<nop>.$inTopic");
    }
...
"parameter *table* is not valid table; the specified table '$tableName' does not exist."
    ...
    if ( defined $yMin ) {
        if ( $scale eq "semilog" && $yMin <= 0 ) {
            return _make_error(
"user set ymin=$yMin is &lt;= 0 which is not valid when scale=semilog"
            );
        }
    }
    $chart->setYmin($yMin);
    ...
    if ( defined $yMax ) {
        if ( $scale eq "semilog" && $yMax <= 0 ) {
            return _make_error(
"user set ymax=$yMax is &lt;= 0 which is not valid when scale=semilog"
            );
        }
    }
    $chart->setYmax($yMax);
    ...
    $chart->setNumYGrids(
        $this->_Parameters->getParameter( "numygrids", $defaultNumYGrids ) );
    ...
    $chart->setDataLabels( split( /[\s,]+/, $dataLabels ) )
      if ( defined $dataLabels );
    ...
    $chart->setImageWidth(
        $this->_Parameters->getParameter( "width", $defaultWidth ) );
    $chart->setImageHeight(
        $this->_Parameters->getParameter( "height", $defaultHeight ) );
    ...
    my $bgcolor =
      $this->_Parameters->getParameter( "bgcolor", $defaultBGcolor );
    ...
    my $gridColor =
      $this->_Parameters->getParameter( "gridcolor", $defaultGridColor );
    $chart->setGridColor( split( /[\s,]+/, $gridColor ) );

    # See if the parameter 'defaultdata' is available.
    my $DataValueDefault =
      $this->_Parameters->getParameter( "defaultdata", $defaultDataValue );
    ...
    if ($legend) {
        ...
        if ( $cnt > 1 ) {
            return _make_error(
                "parameter *legend* specifies multiple ($cnt) rows.");
        }
        if ( $cnt == 0 ) {
            return _make_error(
                "parameter *legend* contains an invalid value '$legend'.");
        }
        @legend = @{ $d[0] };
    ...
    if ( defined($xAxis) ) {
        ...
        if ( abs($xAxisRows) > 1 ) {
            if ( $xAxisColumns > 1 ) {
                return _make_error(
"parameter *xaxis* specifies multiple (${xAxisRows}X$xAxisColumns) rows and columns."
                );
            }
            $columnOrdered = 1;
        }
        ...
        return _make_error(
            "no X axis data found in specified area of table [$xAxis]")
          if ( !@d );
        $chart->setXaxis( @{ $d[0] } );
    }
    else {
...
#return _make_error("parameter *data* value of '$data' is not valid") if (! defined($dataRows));
    ...
    return _make_error("no data found in specified area of table [$data]")
      if ( !@data );
...
"parameter *legend* contains an invalid value '$legend' since it specifies $numLegends legends and there are $numDataSets data sets."
    ...
    $chart->setPointSize(
        $this->_Parameters->getParameter( "pointsize", $defaultPointSize ) );

    # Set the default line width
    $chart->setLineWidth(
        $this->_Parameters->getParameter( "linewidth", $defaultLineWidth ) );
    ...
    my $timestamp = time();
    return
"<img src=\"%ATTACHURL%/$filename?t=$timestamp\" alt=\"$alt\" $options />";
}
...
sub _timeit {
    ...
    if ($removeFiles) {
        for ( my $i = 0 ; $i < $loops ; $i++ ) {
            my ( $dir, $filename ) =
              _make_filename( "area", "timeit_$i", $topic, $web );
            unlink("$dir/$filename");
        }
    }
